...
; gjl
(defvar *maxima-after-load-message*
  (format nil "~&~%  ==== quicklisp quickstart loaded ====~%~%    ~
               To continue with installation within maxima, type quicklisp_install(); ~%"))
...
#+openmcl
  (let ((asdf (qmerge "asdf.lisp"))) ; gjl 2012. ccl with maxima needs this
    (when (probe-file asdf)
      (load asdf)))
  (load (qmerge "setup.lisp"))
  (write-string *maxima-after-initial-setup-message*) ; gjl
  (finish-output))
  ...
  (let ((setup-file (qmerge "setup.lisp")))
    (when (probe-file setup-file)
      (multiple-value-bind (result proceed)
          (with-simple-restart (load-setup "Load ~S" setup-file) ; gjl changed error to format
            (format t "quicklisp has already been installed. load ~s instead."
                   setup-file))
...
(let ((setup-file (qmerge "setup.lisp")))  ; gjl 2012
  (unless (probe-file setup-file)
    (write-string *maxima-after-load-message*)))
