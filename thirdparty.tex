...
\noindent{\bf Calling}
\begin{itemize}
\item[] {\bf ipart}({\it ind1}, {\it ind2}, {\it ...})
  Returns the part of expression {\it e} specified by indices. {\it e} is a mixed representation expression. When used as an lvalue, ipart can be used to assign to a part of an expression. 
...
\vspace{5 pt}
\noindent{\bf Description}
Print all sections of maxdoc documentation in latex format. This does not include other documentation databases, such as the main maxima documentation. 

\vspace{5 pt}

\noindent{\bf Arguments}
   {\tt print\_sections\_latex} requires zero arguments.
...
\item[] {\bf restore}({\it file})
  Reads and returns expressios from the file {\it file}. 
...
\noindent{\bf Implementation}
The algorithm combines a segmented sieve with tables. 
...
\vspace{5 pt}
\noindent{\bf Description}
Pop a value from the current directory stack and chdir to this value. If {\it n} is given, pop {\it n} values and chdir the last value popped. 
...
\vspace{5 pt}
\noindent{\bf Description}
string\_drop is only partially implemented. 
